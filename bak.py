    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            ...
            if len(faces) > 0:
                ...
                # 创建一个与目标图像大小相同的透明背景
                padded_face = np.zeros((img_size, img_size, 4), dtype=np.uint8)

                # 将调整后的头像粘贴到背景中央
                padded_face[(img_size - new_height) // 2: (img_size - new_height) // 2 + new_height,
                (img_size - new_width) // 2: (img_size - new_width) // 2 + new_width] = cv2.cvtColor(resized_face,
                                                                                                     cv2.COLOR_BGR2BGRA)

                output_file = os.path.join(output_dir, file)
                cv2.imwrite(output_file, padded_face)
